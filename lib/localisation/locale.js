...
var _checkPrecision = (value, defValue) => {
    ...
    value = value || 0;
    if (useSymbol)
        return _formatMoney(value, currentLocale.number.places, currentLocale.number.thousandSep, currentLocale.number.decimalSep, currentLocale.number.symbol, currentLocale.number.format);
    else
        return _formatNumber(value, currentLocale.number.places, currentLocale.number.thousandSep, currentLocale.number.decimalSep);
}, _decimal = (value, decimals, symbol) => {
    if (value === null)
        return '';
    value = value || 0;
    var format = symbol ? '%v %s' : '%v';
    return _formatMoney(value, decimals, currentLocale.number.thousandSep, currentLocale.number.decimalSep, symbol, format);
